...
public sealed class ResultPresentationTests
{
    ...
    [Fact]
    public void ErrorCategoriesGroupRowsForEachInspectionLevel()
    {
        var presentation = ResultPresentation.From(Result(5, 0,
        [
            new ScanIssue(IssueSeverity.Error, IssueCode.MissingDirectory, "缺少目录。"),
            new ScanIssue(IssueSeverity.Error, IssueCode.ExtraDirectory, "多出目录。"),
            new ScanIssue(IssueSeverity.Error, IssueCode.CommandUnrecognized, "命令错误。"),
            new ScanIssue(IssueSeverity.Error, IssueCode.CommandUnrecognized, "命令错误。"),
            new ScanIssue(IssueSeverity.Error, IssueCode.BgpPeerNotFound, "BGP错误。"),
        ]));

        var levelOne = presentation.ErrorCategoriesFor(InspectionLevel.DeviceDirectories);
        var levelThree = presentation.ErrorCategoriesFor(InspectionLevel.ExecutionResults);

        Assert.Equal(2, levelOne.Count);
        Assert.Equal(2, levelThree.Count);
        Assert.Contains(levelThree, item => item.CategoryText == "设备不识别命令" && item.Count == 2);
        Assert.Contains(levelThree, item => item.CategoryText == "BGP对等体不存在" && item.Count == 1);
    }
